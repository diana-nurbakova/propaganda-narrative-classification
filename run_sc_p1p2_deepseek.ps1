# =============================================================================
# SC majority with P1 / P2 prompts on DeepSeek V3
...
function Run-Exp($config, $id, $out) {
    ...
    Write-Host "[SC-P1P2-DS] Running: $id ($NRuns run)" -ForegroundColor Green
    & $Python src/H3Prompting/run_multi_experiment.py --config $config --n-runs $NRuns --experiment-id $id --output-dir $out --log
}

Write-Host "`n=== SC k=3 majority + P1/P2 on DeepSeek V3 ===" -ForegroundColor Magenta
