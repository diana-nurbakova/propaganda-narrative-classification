# =============================================================================
# SC majority with P1 / P2 prompts on Llama 3.3 70B via DeepInfra
...
if (-not $env:DEEPINFRA_API_KEY) {
    Write-Host "[ERROR] DEEPINFRA_API_KEY not set." -ForegroundColor Red
    exit 1
}
...
$ModelKey = "deepinfra_llama33_70b"
...
function Run-Exp($config, $id, $out) {
    ...
    Write-Host "[SC-P1P2-DI] Running: $id ($NRuns run)" -ForegroundColor Green
    & $Python src/H3Prompting/run_multi_experiment.py --config $config --n-runs $NRuns --experiment-id $id --output-dir $out --log
}

Write-Host "`n=== SC k=3 majority + P1/P2 on Llama (DeepInfra) ===" -ForegroundColor Cyan
