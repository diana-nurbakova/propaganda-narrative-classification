# =============================================================================
# DeepSeek V3: P1nc / P2nc -- no-conservative variants of P1 / P2
# Tests whether annotation rules + BERTopic keywords (+ ToM) help DeepSeek
# when NOT paired with the "be conservative" instruction that caused the
# Other-rate explosion on PT (89% Other under P1).
...
function Run-Exp($config, $id, $out) {
    ...
    Write-Host "[P1nc/P2nc-DS] Running: $id ($NRuns run)" -ForegroundColor Green
    & $Python src/H3Prompting/run_multi_experiment.py --config $config --n-runs $NRuns --experiment-id $id --output-dir $out --log
}

Write-Host "`n=== DeepSeek: P1nc / P2nc (no-conservative variants) ===" -ForegroundColor Magenta
...
Write-Host "Compare with existing DeepSeek results:" -ForegroundColor Cyan
Write-Host "  P1 (with conservative block)  vs  P1nc (without)"
Write-Host "  P2 (with conservative block)  vs  P2nc (without)"
Write-Host "If P1nc >> P1: conservative block was the problem for DeepSeek"
